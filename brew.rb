...
if linux?
  HOMEBREW_PREFIX = ENV['HOMEBREW_PREFIX'] || "#{ENV['HOME']}/.linuxbrew"
  HOMEBREW_CACHE = ENV['HOMEBREW_CACHE'] || "#{ENV['HOME']}/.cache/Homebrew"
  BREW_REPOSITORY = ENV['BREW_REPOSITORY'] || "https://github.com/Homebrew/linuxbrew"
else
  HOMEBREW_PREFIX = ENV['HOMEBREW_PREFIX'] || "/usr/local"
  HOMEBREW_CACHE = ENV['HOMEBREW_CACHE'] || "/Library/Caches/Homebrew"
  BREW_REPOSITORY = ENV['BREW_REPOSITORY'] || "https://github.com/Homebrew/homebrew"
end
...
abort <<-EOABORT unless linux? || `groups`.split.include?("admin")
...
EOABORT
abort <<-EOABORT if mac? and `/usr/bin/xcrun clang 2>&1` =~ /license/ && !$?.success?
...
wait_for_user if STDIN.tty? and not ARGV.include?("--batch")
...
Dir.chdir HOMEBREW_PREFIX do
  if git
    ...
    system git, "config", "remote.origin.url", BREW_REPOSITORY
    ...
    curl_flags = "fsSL"
    system "/bin/bash -o pipefail -c '/usr/bin/curl -#{curl_flags} #{BREW_REPOSITORY}/tarball/master | /usr/bin/tar xz -m --strip 1'"
  end
end
